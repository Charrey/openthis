...
, __global float* C
    ...
    float acc = 0.0f;
    for (int k=0; k<K; k++) {
        acc += A[k + K*globalRow] * B[globalCol + N*k];
    }
...
, volatile __global int* count
    ...
    // Thread identifiers
    const int begin = get_global_id(0) * Mn; // Row ID of M (0..Mn)
    const int end = begin + Mn;

    // Compute a row (loop over K)
    int sum = 0;
...
, __global float* V
    ...
    for (int split = Vn/2; split > 0; split >>= 1) {
        float value = V[index + split];
        barrier(CLK_GLOBAL_MEM_FENCE);

        if (index < split) V[index] += value;
        barrier(CLK_GLOBAL_MEM_FENCE);
    }
}
